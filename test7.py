...
def get_branches(tree, location: list):
    expr1 = tree.var_name
    expr2 = ".".join([f"nodes[{idx}]" for idx in location])
    expr3 = "nodes"
    expr_list = [expr1, expr2, expr3] if expr2 else [expr1, expr3]
...
if __name__ == "__main__":
    ...
    print(f'tree_main: {tree_main}')ffffff
    print(f'tree_counter: {tree_counter}')
    print(f'stack: {stack}')

    location = []

    print(get_value(tree_main, location))

    location_previous = deepcopy(location)
    nums = stack[len(location_previous)][0]
    val = nums.pop(0)
    print(stack)
    ...
    print()
    print()
    print(get_branches(tree_main, []))
    print(len(get_branches(tree_main, [])))
    print(get_branches(tree_main, [0]))
    print(len(get_branches(tree_main, [0])))
    print()
    print()
    print()
    ...
    def one_step_deeper(location):
        location_previous = deepcopy(location)
        nums = stack[len(location_previous)][0]
        val = nums.pop(0) # stack の操作でもある
        location = add_node(tree_main, val, location_previous, return_new_location=True)
        ...
        if not get_branches(tree_main(location_previous)):
            ops = deepcopy(operators)
        elif ops
